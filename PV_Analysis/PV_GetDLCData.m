...
function [dlcFrame, dlcHead, dlcBody, dlcTail] = PV_GetDLCData(expName)
    ...
    % Get the unique .mp4 file in the directory
    ...
    % Read video and get total frame number
    videoObj = VideoReader(fullfile(expPath, uniqueMp4File));
    totalFrames = videoObj.NumFrames;

    % Get the middle frame
    middleFrameIndex = round(totalFrames / 2);
    middleFrame = read(videoObj, middleFrameIndex);
    ...
    % Overlay scatter plots of dlcBody points
    scatter(dlcBody(:, 1), dlcBody(:, 2), 'r', 'filled'); % Assuming dlcBody has x and y coordinates
