...
while cap.isOpened():
    ...
    dark_contours = list(dark_contours)
    # Find the largest dark contour, smaller than circle area
    largest_dark_contour = None
    largest_dark_area = 0

    # Filter out contours larger than the circle area
    dark_contours = [contour for contour in dark_contours if cv2.contourArea(contour) <= np.pi * (radius * 0.9) ** 2]

    for dark_contour in dark_contours:
        if cv2.contourArea(dark_contour) > largest_dark_area:
            largest_dark_contour = dark_contour
            largest_dark_area = cv2.contourArea(largest_dark_contour)
    ...
    if largest_dark_contour is not None:
        ...
        darker_contours = list(darker_contours)
        # Find the largest darker contour, smaller than the dark bounding box area
        largest_darker_contour = None
        largest_darker_area = 0
        darker_contours = [darker_contour for darker_contour in darker_contours if cv2.contourArea(darker_contour) <= w * h * 0.9]
        for darker_contour in darker_contours:
            if cv2.contourArea(darker_contour) > largest_darker_area:
                largest_darker_contour = darker_contour
                largest_darker_area = cv2.contourArea(largest_darker_contour)
        ...
        if largest_darker_contour is not None:
            ...
            cv2.drawContours(dark_contour_frame[y:y+h, x:x+w], [largest_darker_contour], -1, (255, 0, 0), 2)  # Blue contour
    ...
    if laser_status == "ON":
        laser_color = (0, 255, 0)  # Green for ON
    elif laser_status == "OFF":
        laser_color = (255, 255, 255) # White for OFF

    # Display current time
    cv2.putText(frame, f"Time: {current_time.strftime("%H:%M:%S.%f")[:-3]}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2) # White text
    # Display state text
    cv2.putText(frame, f"Movement: {state}", (10, 90), cv2.FONT_HERSHEY_SIMPLEX, 2, text_color, 2) # Green text for Move, Red text for Stop
    # Display movement threshold
    cv2.putText(frame, f"(Threshold: {movement_threshold:.2f})", (10, 140), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2) # White text
    # Display dark, darker states
    cv2.putText(frame, f"Dark: {dark_state}, Darker: {darker_state}", (10, 170), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2) # White text
    # Display laser status
    cv2.putText(frame, f"Laser: {laser_status}", (10, 200), cv2.FONT_HERSHEY_SIMPLEX, 1, laser_color, 2) # Green text for ON, White text for OFF
