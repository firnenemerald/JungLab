function [Totdata] = fn_behavcluster_OF_v3(DLCTime, DLCnose, DLCcentre, DLCtail, ArenaSize)

% Apply calibration to raw coordinates
calib = 170 / ArenaSize; % Adjust 150 to your standard reference dimension

% Scale the raw coordinates
nose_raw = DLCnose(:, 1:2) * calib;
centre_raw = DLCcentre(:, 1:2) * calib;
tail_raw = DLCtail(:, 1:2) * calib;

% Extract confidence scores
nose_confidence = DLCnose(:, 3);
centre_confidence = DLCcentre(:, 3);
tail_confidence = DLCtail(:, 3);

% Frame numbers
DLCframe_raw = DLCTime;
...
Tvhigh = fn_gaussian_RNN(fn_savgol_RNN(tail_v, 3, 21), 20, 6);
Cvhigh_z = zscore(Cvhigh);
Nvhigh_z = zscore(Nvhigh);
Tvhigh_z = zscore(Tvhigh);
...
t_ipsidur=t_ipsi(:,2)-t_ipsi(:,1);
t_ipsi=t_ipsi(t_ipsidur>1,:); % 20? more than 1s

if ~isempty(t_cont)
    t_contdur=t_cont(:,2)-t_cont(:,1);
    t_cont=t_cont(t_contdur>1,:); %more than 1s
end
